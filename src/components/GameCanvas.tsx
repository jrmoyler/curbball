  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (!isBallFlying) {
        ...
        setCurbCoins((prev) =>
          prev.map((coin) => {
            const radius = Math.max(10, nextLayout.screenW * 0.025);
            return {
              ...coin,
              radius,
              x: Math.max(radius, Math.min(nextLayout.screenW - radius, coin.x)),
              y: Math.max(nextLayout.roadTopY + radius, Math.min(nextLayout.roadBottomY - radius, coin.y)),
            };
          }),
        );
